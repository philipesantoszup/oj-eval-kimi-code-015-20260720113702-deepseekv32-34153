...
const int HASH_SIZE = 65536; // 2^16 buckets
...
private:
    fstream file;
    int hash_table[HASH_SIZE]; // in-memory hash table
    ...
    int allocateEntry() {
        if (free_head != INVALID_INDEX) {
            int index = free_head;
            // Read the free entry to get next free pointer
            Entry free_entry;
            readEntry(index, free_entry);
            free_head = free_entry.next;
            return index;
        } else {
            file.seekp(0, ios::end);
            int index = (static_cast<int>(file.tellp()) - sizeof(int)) / sizeof(Entry);
            return index;
        }
    }
    ...
    void freeEntry(int index) {
        ...
        entry.value = 0;
        entry.next = free_head; // Store free list pointer here
        entry.valid = false;
    ...
    void readEntry(int index, Entry& entry) {
        file.seekg(sizeof(int) + index * sizeof(Entry));
        file.read(reinterpret_cast<char*>(&entry), sizeof(entry));
    }
    
    void writeEntry(int index, const Entry& entry) {
        file.seekp(sizeof(int) + index * sizeof(Entry));
        file.write(reinterpret_cast<const char*>(&entry), sizeof(entry));
        file.flush();
    }
    
    void updateNextPointer(int index, int new_next) {
        // Read the entry
        Entry entry;
        readEntry(index, entry);
        // Update only the next pointer
        entry.next = new_next;
        // Write back
        file.seekp(sizeof(int) + index * sizeof(Entry));
        file.write(reinterpret_cast<const char*>(&entry), sizeof(entry));
...
public:
    DiskHashTable() : free_head(INVALID_INDEX) {
        // Initialize hash table to -1
        fill(hash_table, hash_table + HASH_SIZE, INVALID_INDEX);
        
        if (filesystem::exists(DATA_FILE)) {
            file.open(DATA_FILE, ios::binary | ios::in | ios::out);
            // Read free_head from beginning of file
            file.seekg(0);
    ...
    ~DiskHashTable() {
        if (file.is_open()) {
            // Update free_head at beginning of file
            file.seekp(0);
            file.write(reinterpret_cast<const char*>(&free_head), sizeof(free_head));
            file.close();
        }
        ...
        while (curr != INVALID_INDEX) {
            ...
            if (entry.matches(key, value)) {
                if (prev == INVALID_INDEX) {
                    ...
                    updateNextPointer(prev, entry.next);
                }
